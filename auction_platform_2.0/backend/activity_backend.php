            ...
            foreach ($bid_items as $bid_item) {
                ...
                if (mysqli_num_rows($result)>0) {
                    while ($row = $result->fetch_row()) {
                        ...
                        $query2 = "SELECT img_url from `listed_items` where item_id='$bid_item'";
                        $result2 = mysqli_query($connection, $query2);
                        $row2 = $result2->fetch_row();
                        if (!in_array(array($row1[1],$row[3]), $buyers_and_comments_with_the_same_items)) {
                            array_push($buyers_and_comments_with_the_same_items, array($row1[1],$row[3],$row2[0]));
                        }
                    }
                }
            }
